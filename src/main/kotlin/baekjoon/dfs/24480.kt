...
fun main() {
    ...
    graph.forEach { (_: Int, l: MutableList<Int>) -> l.sortWith(Comparator.reverseOrder()) }

    val isVisited = (1..n).map { 0 }.toMutableList()
    dfs2( r - 1, graph, isVisited)

    isVisited.forEach{ println(it) }
}

var count2 = 1
fun dfs2(index: Int, graph: Map<Int, List<Int>>, isVisited: MutableList<Int>) {
    isVisited[index] = count2++
    ...
    for (nextIndex in graph.getOrDefault(index, emptyList())) {
        ...
        dfs2(nextIndex, graph, isVisited)
    }
}
