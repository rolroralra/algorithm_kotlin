...
fun main() {
    ...
    println(lcsLength(input1, input2))
}

private fun lcsLength(input1: String, input2: String): Int {
    ...
    for (i in input1.indices) {
        for (j in input2.indices) {
            if (input1[i] == input2[j]) {
                ...
                dp[i][j] = maxOf(if (i == 0) 0 else dp[i - 1][j], if (j == 0) 0 else dp[i][j - 1])
            }
        }
    }
...
private fun lcs(input1: String, input2: String): String {
    ...
    while (sb.length < lcsLength) {
        val (i2, j2, isMatch) = nextMove(i, j, dp, sb)
...
private fun nextMove(i: Int, j: Int, lcs: List<List<Int>>, sb: StringBuilder): Triple<Int, Int, Boolean> {
